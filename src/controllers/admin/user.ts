import { userModel, followFolloweeModel } from '@models/index';
...
function details(query: any): Promise<any> {
    return new Promise(async (resolve, reject) => {
        try {
            const details: any = await userModel.findOne({ _id: query.userId, isDelete: false })
            if (details) {
                const followerCount = await followFolloweeModel.count({ followeeId: details._id })
                const followeeCount = await followFolloweeModel.count({ followerId: details._id })
                resolve({ response: details, followerCount: followerCount, followeeCount: followeeCount });
            } else {
